...
function TheaterList(props) {
    ...
    function followThis(event, val) {
        ...
        if (currentUser?.user?._source?.user_role === 'theater') {
            NotificationManager.error('A Theater can only follow Businesses');
        } else {
            ...
            _restServices.followBusinessOrTheater(currentUser?.user, val?._index, val?._id).then(
                res => {
                    ...
                    if (res['status'] === 1) {
                        setOpen(false)
                        NotificationManager.success(`You started following ${val?._source?.theater_name}`, 'Success');
    ...
    return (
        <>
            <div className="single-theater" onClick={(e) => { clickMe(e, props.data) }} >
                <h6>{props.data?._source?.theater_name}</h6>
                {props.data?._source?.theater_desc.length > 10 ?
                    `${props.data?._source?.theater_desc.substring(0, 57)}...` : props.data?._source?.theater_desc
                }
            </div>
